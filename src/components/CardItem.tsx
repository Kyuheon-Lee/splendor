...
/** 카드 비용 아이템 컴포넌트 */
const CostItem = ({ color, amount }: { color: string; amount: number }) => (
  <div className={cn(
    "w-7 h-7 rounded-full flex items-center justify-center shadow-lg border-2 border-white/80",
...
export default function CardItem({ card, onClick }: CardItemProps) {
  ...
  return (
    <div 
      onClick={onClick}
      className={cn(
        "w-32 h-48 bg-zinc-100 rounded-xl flex flex-col relative overflow-hidden group cursor-pointer hover:-translate-y-4 transition-all duration-500 ease-out border-2",
        card.level === 1 && "border-zinc-200 shadow-md",
        card.level === 2 && "border-white/60 shadow-xl scale-105 ring-1 ring-white/30",
        card.level === 3 && "border-amber-400 shadow-[0_0_30px_rgba(251,191,36,0.5)] scale-110 ring-2 ring-amber-400"
      )}
    >
      {/* 카드 배경 그라데이션 */}
      <div className="absolute inset-0 opacity-20 bg-gradient-to-b from-white to-zinc-400" />
      <div className={cn("absolute inset-0 opacity-10", theme.bg)} />
      ...
      <div className="absolute top-0 w-full p-2 flex justify-between items-start z-20">
        ...
        <div className={cn(
          "w-10 h-10 rounded-full shadow-lg flex items-center justify-center text-2xl border-2 border-white/50 transform group-hover:rotate-12 transition-transform",
          theme.bg
        )}>
          {elementIcon}
        </div>
      ...
      <div className="flex-1 flex items-center justify-center relative z-10">
        <div className={cn(
          "absolute w-28 h-28 rounded-full blur-3xl opacity-30 group-hover:scale-150 transition-transform duration-1000",
          theme.bg
        )} />
        <div className={cn(
          "transition-all duration-500 group-hover:scale-110 drop-shadow-[0_10px_10px_rgba(0,0,0,0.3)]",
          card.level === 1 && "text-6xl opacity-70",
          card.level === 2 && "text-7xl opacity-90",
          card.level === 3 && "text-8xl opacity-100"
        )}>
          {elementIcon}
        </div>
        {card.level === 3 && (
      ...
      <div className="absolute bottom-2 left-2 flex flex-col-reverse gap-1 z-20">
        {Object.entries(card.cost).map(([color, amount]) => (
          <CostItem key={color} color={color} amount={amount} />
        ))}
      </div>
