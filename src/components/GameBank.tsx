...
const BankTokenItem = ({ 
  ...
  onSelect: (color: Exclude<GemColor, 'gold'>) => void;
}) => {
  ...
  return (
    <div 
      onClick={() => !isGold && onSelect(color as Exclude<GemColor, 'gold'>)}
      className={cn(
        "flex flex-col items-center gap-2 group transition-all",
        isGold ? "cursor-not-allowed opacity-40" : "cursor-pointer"
      )}
    >
      <div className={cn(
        "w-14 h-14 rounded-full flex flex-col items-center justify-center font-black text-xl shadow-[inset_0_-4px_8px_rgba(0,0,0,0.3)] border-2 transition-all duration-300 group-hover:scale-110 relative",
        theme.bg, 
        theme.text,
        selectedCount > 0 
          ? "ring-4 ring-amber-500 ring-offset-4 ring-offset-zinc-800 scale-110 border-white" 
          : "border-white/20"
      )}>
        <div className="absolute top-1 left-2 w-3 h-2 bg-white/30 rounded-full rotate-[-45deg]" />
        <span className="text-sm opacity-50 mb-0.5">
          {'icon' in theme ? (theme as { icon: string }).icon : (theme as { icons: string[] }).icons[0]}
        </span>
...
export default function GameBank() {
  ...
  return (
    <div className={cn(
      "bg-zinc-800 p-6 rounded-2xl border border-zinc-700 shadow-xl overflow-hidden relative flex flex-col transition-opacity",
      ...
      <div className="grid grid-cols-3 gap-y-8 gap-x-4">
        {Object.entries(tokens).map(([color, count]) => (
          <BankTokenItem 
            ...
            selectedCount={color === 'gold' ? 0 : (selectedTokens[color as Exclude<GemColor, 'gold'>] || 0)}
      ...
      <div className={cn(
        ...
        isSelectionValid ? "max-h-20 opacity-100" : "max-h-0 opacity-0 pointer-events-none"
      )}>
        <button 
